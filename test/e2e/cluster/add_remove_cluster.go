...
var _ = Describe("Test to check adding/removing a new cluster to existing setup", func() {
	...
	It("Should be able to add and remove 3rd cluster", func() {
		...
		enginePod := f.AwaitPodsByAppLabel(framework.ClusterC, framework.SubmarinerEngine, "submariner", 1).Items[0]
		...
		dataplane.RunNoConnectivityTest(f, false, framework.GatewayNode, framework.NonGatewayNode, framework.ClusterA, framework.ClusterC)
	})
})
