...
var _ = PDescribe("[expansion] Test expanding/shrinking an existing cluster fleet", func() {
	...
	It("Should be able to add and remove third cluster", func() {
		...
		dataplane.RunConnectivityTest(f, false, false, framework.GatewayNode, framework.GatewayNode, framework.ClusterC, framework.ClusterB)
		dataplane.RunConnectivityTest(f, true, false, framework.NonGatewayNode, framework.NonGatewayNode, framework.ClusterC, framework.ClusterA)
