...
func testOneGatewayNode(f *framework.Framework) {
	...
	dataplane.RunConnectivityTest(f, false, false, framework.GatewayNode, framework.GatewayNode, framework.ClusterA, framework.ClusterB)

	By(fmt.Sprintf("Verifying TCP connectivity from non-gateway node on %q to non-gateway node on %q", clusterBName, clusterAName))
	dataplane.RunConnectivityTest(f, false, false, framework.NonGatewayNode, framework.NonGatewayNode, framework.ClusterA, framework.ClusterB)
}
