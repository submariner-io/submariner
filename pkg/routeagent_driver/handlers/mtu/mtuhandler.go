...
func (h *mtuHandler) Init(_ context.Context) error {
	...
	if err := h.pFilter.CreateIPHookChainIfNotExists(&packetfilter.ChainIPHook{
		...
		return errors.Wrapf(err, "error creating IPHookChain chain %s", chains.SmPostRoutingMss)
	}
	...
	if err := h.pFilter.AppendUnique(h.tableType, chains.SmPostRoutingMss, ruleSpecSource); err != nil {
		return errors.Wrapf(err, "error appending packetfilter rule %q", ruleSpecSource)
	}

	if err := h.pFilter.AppendUnique(h.tableType, chains.SmPostRoutingMss, ruleSpecDest); err != nil {
...
func (h *mtuHandler) Uninstall() error {
	...
	logError(h.pFilter.ClearChain(h.tableType, chains.SmPostRoutingMss),
		"Error flushing chain %q of table type %q", chains.SmPostRoutingMss, h.tableType.String())

	logger.Infof("Deleting IPHook chain %q of table type %q", chains.SmPostRoutingMss, h.tableType.String())
	...
	logError(h.pFilter.DeleteIPHookChain(&packetfilter.ChainIPHook{
		...
		Hook:     packetfilter.ChainHookPostrouting,
		Priority: packetfilter.ChainPriorityFirst,
	}), "Error deleting IP hook chain %q of table type %q", chains.SmPostRoutingMss, h.tableType.String())
...
func (h *mtuHandler) forceMssClamping(endpoint *submV1.Endpoint) error {
	...
	if err := h.pFilter.UpdateChainRules(h.tableType, chains.SmPostRoutingMss, rules); err != nil {
		return errors.Wrapf(err, "error updating chain %s table type %q", chains.SmPostRoutingMss, h.tableType.String())
	}
