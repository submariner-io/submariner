...
func (kp *SyncHandler) NodeCreated(node *k8sV1.Node) error {
	...
	for i, addr := range node.Status.Addresses {
		if addr.Type == k8sV1.NodeInternalIP {
			...
			val, ok := node.Labels["submariner.io/gateway"]
			if !ok {
				kp.populateRemoteVtepIps(node.Status.Addresses[i].Address, Add, false)
			} else {
				gwEnabled, err := strconv.ParseBool(val)
				if err != nil {
					klog.Fatalf("submariner.io/gateway exists but value %s cannot be parsed: %v", val, err)
				}

				kp.populateRemoteVtepIps(node.Status.Addresses[i].Address, Add, gwEnabled)
			}
		}
	}
