...
type NonGatewayRouteInformer interface {
	...
	Lister() v1.NonGatewayRouteLister
}
...
func NewNonGatewayRouteInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredNonGatewayRouteInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.SubmarinerV1().NonGatewayRoutes(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.SubmarinerV1().NonGatewayRoutes(namespace).Watch(context.TODO(), options)
			},
		},
		&submarineriov1.NonGatewayRoute{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *nonGatewayRouteInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredNonGatewayRouteInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *nonGatewayRouteInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&submarineriov1.NonGatewayRoute{}, f.defaultInformer)
}

func (f *nonGatewayRouteInformer) Lister() v1.NonGatewayRouteLister {
	return v1.NewNonGatewayRouteLister(f.Informer().GetIndexer())
}
