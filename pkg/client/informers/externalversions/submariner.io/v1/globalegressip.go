...
type GlobalEgressIPInformer interface {
	...
	Lister() v1.GlobalEgressIPLister
}
...
func NewGlobalEgressIPInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredGlobalEgressIPInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
				...
				return client.SubmarinerV1().GlobalEgressIPs(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.SubmarinerV1().GlobalEgressIPs(namespace).Watch(context.TODO(), options)
			},
		},
		&submarineriov1.GlobalEgressIP{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *globalEgressIPInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredGlobalEgressIPInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *globalEgressIPInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&submarineriov1.GlobalEgressIP{}, f.defaultInformer)
}

func (f *globalEgressIPInformer) Lister() v1.GlobalEgressIPLister {
	return v1.NewGlobalEgressIPLister(f.Informer().GetIndexer())
}
