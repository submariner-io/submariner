...
func (a *amneziawgDriver) GetConnections() ([]v1.Connection, error) {
	...
	for i := range d.Peers {
		...
		connection := a.connectionByKey(&key)
		if connection == nil {
			logger.Warningf("Found unknown peer with key %s, removing", key)

			if err := a.removePeer(&key); err != nil {
				logger.Errorf(err, "Could not delete AmneziaWG peer with key %s, ignoring", key)
			}

			continue
		}

		a.updateConnectionForPeer(&d.Peers[i], connection)
...
func (a *amneziawgDriver) connectionByKey(key *wgtypes.Key) *v1.Connection {
	for i := range a.connections {
		...
		k, _ := keyFromSpec(&a.connections[i].Endpoint)
...
// Update logic, based on delta from last check: good state requires a handshake and traffic;
// no handshake or a stale handshake moves the connection toward Connecting / ConnectionError.
func (a *amneziawgDriver) updateConnectionForPeer(p *wgtypes.Peer, connection *v1.Connection) {
	now := int64(time.Nanosecond) * time.Now().UnixNano() / int64(time.Millisecond)

	pollMS := StatusPollInterval.Milliseconds() + 2 // +2 for rounding
	...
	// Expect Rx/Tx within the peer keepalive; if disabled, use the poll interval.
	trafficWindow := a.keepAlive
	if trafficWindow <= 0 {
		trafficWindow = StatusPollInterval
	}

	trafficMS := trafficWindow.Milliseconds() + 2
	...
	if p.LastHandshakeTime.IsZero() {
		if lc > HandshakeTimeout.Milliseconds() {
			...
			cable.RecordConnection(cableDriverName, &a.localEndpoint, &connection.Endpoint, string(connection.Status), false, connectionFamily)
		} else if tx > 0 || rx > 0 {
	...
	if tx > 0 || rx > 0 {
		...
		cable.RecordConnection(cableDriverName, &a.localEndpoint, &connection.Endpoint, string(connection.Status), false, connectionFamily)
		saveAndRecordPeerTraffic(&a.localEndpoint, &connection.Endpoint, now, p.TransmitBytes, p.ReceiveBytes, connectionFamily)
	...
	if lc < 2*trafficMS {
		// Grace period until keepalive (or poll floor) should have produced traffic.
...
func (a *amneziawgDriver) updatePeerStatus(c *v1.Connection, key *wgtypes.Key) {
	p, err := a.peerByKey(key)
	if err != nil {
		c.SetStatus(v1.ConnectionError, "cannot fetch status for peer %s: %v", key, err)
		cable.RecordConnection(cableDriverName, &a.localEndpoint, &c.Endpoint, string(c.Status), false, c.GetFamily())

		return
	}

	a.updateConnectionForPeer(p, c)
}
