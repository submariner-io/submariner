...
var _ = Describe("Endpoint monitoring", func() {
	...
	When("a remote Endpoint with non-overlapping CIDRs is created then removed", func() {
		It("should add/remove appropriate IP table rule(s)", func() {
			...
			t.pFilter.AwaitRule("nat", constants.SmGlobalnetMarkChain, ContainSubstring(remoteCIDR))

			Expect(t.endpoints.Delete(context.TODO(), endpoint.Name, metav1.DeleteOptions{})).To(Succeed())
			t.pFilter.AwaitNoRule("nat", constants.SmGlobalnetMarkChain, ContainSubstring(remoteCIDR))
		})
	})
	...
	When("a remote Endpoint with an overlapping CIDR is created", func() {
		It("should not add expected IP table rule(s)", func() {
			...
			t.pFilter.AwaitNoRule("nat", constants.SmGlobalnetMarkChain, ContainSubstring(localCIDR))
		})
	})
})
...
func (t *gatewayMonitorTestDriver) start() {
	...
	t.pFilter.AwaitChain("nat", constants.SmGlobalnetMarkChain)
}
...
func (t *gatewayMonitorTestDriver) awaitGlobalnetChains() {
	t.pFilter.AwaitChain("nat", constants.SmGlobalnetIngressChain)
	t.pFilter.AwaitChain("nat", constants.SmGlobalnetEgressChain)
	t.pFilter.AwaitChain("nat", routeAgent.SmPostRoutingChain)
	t.pFilter.AwaitChain("nat", constants.SmGlobalnetMarkChain)
}

func (t *gatewayMonitorTestDriver) awaitNoGlobalnetChains() {
	t.pFilter.AwaitNoChain("nat", constants.SmGlobalnetIngressChain)
	t.pFilter.AwaitNoChain("nat", constants.SmGlobalnetEgressChain)
	t.pFilter.AwaitNoChain("nat", constants.SmGlobalnetMarkChain)
}
