...
type engine struct {
	...
	endpointNatDetails chan *natdiscovery.NATEndpointInfo
}
...
func (i *engine) SetupNATDiscovery(natDiscovery natdiscovery.Interface) {
	...
	i.endpointNatDetails = make(chan *natdiscovery.NATEndpointInfo, 100)
	natDiscovery.SetReadyChannel(i.endpointNatDetails)
	go func() {
		for natEndpointInfo := range i.endpointNatDetails {
			err := i.installCableWithNATDetails(natEndpointInfo)
			if err != nil {
...
func (i *engine) installCableWithNATDetails(rnat *natdiscovery.NATEndpointInfo) error {
	...
	for _, active := range activeConnections {
		...
		if active.Endpoint.CableName == endpoint.Spec.CableName {
			...
			if active.UsedIP == rnat.UseIP && active.NAT == rnat.UseNAT {
